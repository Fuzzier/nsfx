 ...
 *
 * # Concept
 *   A log source generates runtime information, and feed them into log sinks.
 *
 ...
 *   The information is represented as a collection of named values.
 *
 *   A log record is a collection of named values.
 *   A log source generates log records, and passes them to log sinks.
 *   And the log sinks are responsible to process the log records, e.g., modify,
 *   forward, format, display or store the log records.
 *
 * # Record
 ...
 *
 * # Source and sink
 *   In order to process log records, a log sink usually have to understand
 ...
 *   Each of the log sink may perform certain operations upon the log record.
 *   In such perspective, each log sink in the chain is a log source with
 *   respective to the log sinks after it.
 *
 *   The library provides \c ILog interface that provides a single method
 *   \c ILog::Commit() that commits a log record.
 *   The initial log record is generated by users and passed to the first log
 *   sink of the chain.
 *
 * # Attribute
